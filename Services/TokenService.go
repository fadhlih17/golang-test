...
func (u *TokenService) DisableToken(ctx context.Context, token string) (interface{}, error) {
	var tokenService = model.Token{
		Token:   token,
		IsValid: 0,
	}
	res, err := u.tokenRepo.UpdateToken(ctx, token, &tokenService)
	return res, err
}
